...
loadData("json/config.json.php", ["json/prompts-" + user_prompt_lang + ".json.php", "json/lang.json.php", "json/badwords.json.php", "json/tarot.json.php"]);
		  ...
		  source.addEventListener('message', function (e) {
		    ...
		    if (typeof data === 'string') {
		      if (data.startsWith('[ERROR]')) {
		        ...
		        return false;
			  } else if (data.trim() == '[PREMIUM]') {
		        $(".cursor").remove();
		        str = $(`.${randomID}`).html();
		        str = "You need to register or login to continue talking to the oracle <a href='/login' class='in_chat_button'>Login</a>."; //escapeHtml(str); //mxl:${lang["translate"][lang_index].error_chatmsg_premium_required}
		        $(`.${randomID}`).html(str);
		        $(`.chat_${randomID} .chat-audio`).fadeIn('slow');
		        enableChat();
		        scrollChatBottom();
